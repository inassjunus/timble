...
func (usecase PremiumUc) Grant(ctx context.Context, userID uint) error {
	...
	err := usecase.db.UpdateUser(userData)
	if err != nil {
		return errors.WithStack(err)
	}
	usecase.cache.Set(buildPremiumCacheKey(userID), []byte(PREMIUM_TRUE_STRING), premiumExpCache)
	return nil
}
...
func (usecase PremiumUc) Unsubscribe(ctx context.Context, userID uint) error {
	...
	usecase.cache.Set(buildPremiumCacheKey(userID), []byte(PREMIUM_FALSE_STRING), premiumExpCache)
	return nil
}
